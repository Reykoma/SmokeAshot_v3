...
Здесь код на уровне логики. Как будто содержание в книге.
Отображено на более высоком уровне что делает программа
А сам код низкоуровневый. Который непосредственно уже выполняет саму работу находится в классе "SAtools"
...
//        Закрепляем шапку, чтобы не мешала при скроле
        ...
        //Это необходимо так как на сайте динамическая подгрузка. Например если вы откроете главную рбк. У вас еще не подгружен подвал. и сделать скриншот подвала невозможно.
        //а для того чтобы это стало возможно, необходимо именно проскролить всю страницу, чтобы прогрузился пдовал
        SAtools.scrollForFooter();

/*
Здесь применяется библиотека https://github.com/pazone/ashot
Которая позволяет делать скриншоты отдельных элементов.
 */
...
//        Сравниваем
        ...
        //Проверка что скрин прода и стейджа идентичны.
        SAtools.asserting();
    }
